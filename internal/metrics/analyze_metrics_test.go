// internal/metrics/analyze_metrics_test.go
package metrics
...
func TestConvertModelMetrics(t *testing.T) {
	models := []ModelMetrics{
		{
			ModelName: "m1",
			OverallStats: RunningAggregatedStats{
				TotalRequests: 2,
				TotalDurationMillis: RunningStat{Mean: 100, Min: 90, Max: 110},
				TTFTMillis:          RunningStat{Mean: 10, Min: 9, Max: 11},
				TokensPerSecond:     RunningStat{Mean: 5, Min: 4, Max: 6},
				InputTokens:         RunningStat{Mean: 7, Min: 6, Max: 8},
				OutputTokens:        RunningStat{Mean: 3, Min: 2, Max: 4},
			},
		},
	}
...
func TestBuildStatsFromIterations(t *testing.T) {
	iters := []Iteration{
		{Iteration: 1, Stats: Stats{TotalExecutionTime: 10, TimeToFirstToken: 3, TokensPerSecond: 2, InputTokenCount: 1, OutputTokenCount: 2}},
		{Iteration: 2, Stats: Stats{TotalExecutionTime: 20, TimeToFirstToken: 4, TokensPerSecond: 4, InputTokenCount: 2, OutputTokenCount: 3}},
	}
...
func TestLoadBenchmarksDir(t *testing.T) {
	...
	results, err := loadBenchmarksDirForTest(dir)
	if err != nil {
