...
func (m *pipelineModel) pipelineView() string {
	...
	parts = append(parts, m.renderStageColumns())
...
func (m *pipelineModel) renderStageColumns() string {
	colWidth := max(30, (m.width-8)/pipelineStageCount)
	var columns []string

	for i, stage := range m.stages {
		column := m.renderStageColumn(stage, colWidth)
...
func (m *pipelineModel) renderStageColumn(stage pipelineStage, colWidth int) string {
...
func (m *pipelineModel) renderStageBody(stage pipelineStage, colWidth int) string {
	...
	case pipelineStageViewHandoff:
		...
		preview := fmt.Sprintf("%s", stage.handoff.preview)
	...
	default:
		if stage.finalOutput != "" {
			return truncateToWidth(stage.finalOutput, colWidth-4)
		}
...
func (m *pipelineModel) handleStageDone(msg pipelineStageDoneMsg) tea.Cmd {
	...
	if !m.prepareHandoff(stage) {
		...
		stage.err = fmt.Errorf("json validation failed")
		m.runInProgress = false
		m.viewState = pipelineViewReady
		if m.runCompleted.IsZero() {
			m.runCompleted = time.Now()
...
func (m *pipelineModel) prepareHandoff(stage *pipelineStage) bool {
	...
	preview := truncateRunes(payload, pipelinePreviewRunes)
	summary := ""
	if truncated {
	...
	stage.handoff = pipelineHandoff{
		...
		tokenCount:        min(tokens, pipelineMaxHandoffTokens),
	}
	return true
}
...
func (m *pipelineModel) currentStageProgress() (int, int, string) {
	...
	if m.runInProgress {
		return min(completed+1, totalAssigned), totalAssigned, "Waiting"
	}
	if completed > 0 {
...
func pipelineStreamStageCmd(p *tea.Program, stageIndex int, host Host, modelName string, history []chatMessage, systemPrompt string, parameters Parameters, payload string, jsonMode bool, client *http.Client, timeout time.Duration) tea.Cmd {
	return func() tea.Msg {
		...
		if err != nil {
			cancel()
			return pipelineStageErrorMsg{Stage: stageIndex, Err: err}
		}
		...
		if resp.StatusCode != http.StatusOK {
			...
			resp.Body.Close()
			cancel()
			return pipelineStageErrorMsg{Stage: stageIndex, Err: fmt.Errorf("API error: %s - %s", resp.Status, string(bodyBytes))}
		}
		...
		go func() {
			...
			for {
				...
				if chunk.Message.Content != "" {
					p.Send(pipelineStageChunkMsg{Stage: stageIndex, Content: chunk.Message.Content})
				}
				if chunk.Done {
					finalChunk = chunk
					break
